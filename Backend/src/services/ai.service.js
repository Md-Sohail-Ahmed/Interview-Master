...
async function generateStructuredJson({ prompt, schema, maxAttempts = 3 }) {
    let lastError = null
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
        try {
            ...
            const isRetryable = error?.status === 503 || error?.error?.code === 503
    ...
    const isUnavailable = lastError?.status === 503 || lastError?.error?.code === 503
    ...
    throw new AIServiceError("Failed to generate AI content. Please try again.", 500)
}
